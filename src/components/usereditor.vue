<template>
  <div class="user-editor" :user="user" :userId="userId" :isAdmin="isAdmin" :groups="groups">
    <i-form ref="form" :model="curruser" :rules="rules" label-position="right" :label-width="80">
      ...
      <Form-item label="员工性质" prop="userType">
        <i-select placeholder="请选择" :label-in-value="true" @on-change="typeChange" v-model="curruser.userType">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      typeChange(CurType) {
        console.log(CurType);
        this.curruser.userType = CurType.value;
      },
      resetPwd() {
        if (this.user.email) {
      ...
      save() {
        this.$refs.form.validate(isValidated => {
          ...
          if (o.userType !== c.userType) data.userType = c.userType;
          if (this.isAdmin) {
